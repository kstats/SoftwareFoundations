...
(**DON'T DO THIS PALINDROME EXERCISE**)
...
Inductive ge : nat -> nat -> Prop :=
  ...
  | ge_S : forall n m, (ge n m) -> (ge m (S n)).
(** [] *)

(** **** Exercise: 2 stars (empty_relation) *)
(** Define an inductive binary relation [empty_relation] (on numbers)
    that never holds. *)
Inductive wrong : nat -> nat -> Prop :=
  | false_n_m : forall n m, le 1 0 -> wrong n m.
(*since one will never be less than 0, the relation is always false*)
...
Proof.
  ...
  inversion H1.
  rewrite->H1 in H0.
  rewrite->plus_comm with (n := 0) (m := o') in IHo'.
  rewrite->plus_0_r in IHo'.
  generalize H0.
