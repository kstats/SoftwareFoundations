...
(* No, it does not hold.  Counter example:  t' = [x := v] t1.
t = tapp ((tabs x T t1) v2). If v2 does not have type T, 
the subject expansion property does not hold.*)
[]
*)
...
(** Another pleasant property of the STLC is that types are
    ...
    type. *)
Theorem types_unique : forall Gamma t T, Gamma |- t \in T ->
  forall T', Gamma |- t \in T' -> T = T'.   
Proof.
  ...
  Case "tabs".
    ...
    inversion H2.
    subst.
    subst. inversion H2.  subst. apply IHhas_type in H6. subst. 
    reflexivity.
  Case "tapp".
    intros T' H2.
    inversion H2.
...
and the following typing rule:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
becomes false.  Counter example: tapp t1 t2 ==> zap.  This could return 
either t2 or T, so determinism fails.
      - Progress
remain true
      - Preservation
remain true
...
remains true
      - Preservation
becomes false. Counter example: \x:A is a different type than true, so type 
is not preserved.
[]
*)
...
remains true
      - Progress
becomes false. tapp (tapp (tapp t1 t2) t3) t4 is stuck, which means
progress has become false.
      - Preservation
remains true
[]
*)
...
(* I plan to go back and complete this next week as I have too many
projects in other classes all due tonight :/ *)
(** [] *)
