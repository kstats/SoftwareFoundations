...
(* 1 star exercises aren't required. *)
...
Proof.
  intros n m p.
  rewrite -> plus_comm.
(* had to wait to use plus_assoc because easier to apply assertions
if there were two variables, one of which is in the parentheses*)
...
(*the proof worked when i proved this in a lemma, but not when i used it in an
assertion.  Ask in class about why this happened?*)
...
Proof.
  ...
  Case "n = 0".
    simpl. reflexivity.
  Case "n = S n'".
    simpl. rewrite-> IHn'. rewrite->plus_swap. 
    reflexivity.
Qed.
...
Proof.
  ...
  Case "m = S m'".
    simpl. rewrite->IHm'.  rewrite->mult_comm_lemma.
    reflexivity.
Qed.
...
(*below is a copy of my prior assignment*)
Inductive bin : Type :=
  | B : bin
...
Fixpoint convert_to_unary (n : bin): nat :=
  ...
  |Tbo n' => 1 + 2 * convert_to_unary(n') 
  end.


Lemma bin_comm_lemma_2 : forall n : nat,
  n + 0 = n.
Proof.
  induction n as [|n'].
  Case "n = O".
...
Theorem bin_comm : forall b : bin,
    convert_to_unary (increment b) = 1 + convert_to_unary b.
Proof.
  ...
  Case "b = Tb b'".
    simpl. reflexivity. 
  Case "b = Tbo b''".
    simpl. rewrite-> IHb''. repeat rewrite-> bin_comm_lemma_2. 
