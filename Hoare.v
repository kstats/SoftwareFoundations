...
(* if a = X + 1.  X is assigned to x+1 with the statement X ::= a.
That makes X not equal to a, however, since X != X + 1.  So, this 
isn't a True statement.

Any a that involves x but isn't equivalent to x is CEx*)
(** [] *)
...
Example second_ex:
  {{ fun st: state => 0 <= 3 /\ 3 <= 5 }}  
  X ::= (ANum 3)  
  {{ fun st: state => 0 <=  st X /\ st X <= 5 }}.
Proof. 
  eapply hoare_consequence_pre. 
  apply hoare_asgn. 
  unfold assert_implies. 
...
Definition Temp : id := Id 0.
Definition swap_program : com :=
   Temp ::= AId X;; X ::= AId Y;; Y ::= AId X.
...
(* If a was dependent on X, the value of a would vary depending on the value
of x.  This means the value of st changes throughout the program, meaning
it impossible to prove the triple. *)
(** [] *)
...
Proof.
  ...
  Case "Then".
    ...
    unfold bassn, assn_sub, update, assert_implies.
    simpl. intros st [_ H].
    apply le_plus_minus. apply ble_nat_true in H.
    rewrite H. 
    reflexivity.
  Case "Else".
    eapply hoare_consequence_pre. apply hoare_asgn.
...
Theorem hoare_if1 : forall P Q b c,
 ...
 {{P}} (IF1 b THEN c FI) {{Q}}. 
Proof.
  intros P Q b c. intros H1 H2.
  intros st st'. intros H3 H4.
  inversion H3.
  subst.
  apply (H1 st st').
  assumption.
...
Proof. 
  ...
  Case "true".
    ...
    unfold assn_sub. 
    unfold update. 
    simpl. destruct H as [H1 _]. 
    apply H1.
  Case "false".
    eapply hoare_consequence_pre. 
    apply hoare_skip.
    unfold bassn, 
    assert_implies, assn_sub, update.
    intros st [H1 H2].
    simpl in H2.
    unfold negb in H2. 
    destruct (st Y).
