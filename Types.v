...
Proof with eauto.
  ...
  apply IHstep in H0. constructor ; try assumption.
  inversion H3 ; subst. assumption. 
  inversion H1 ; subst.
  assumption.  constructor.
  apply IHstep. apply H0.
  constructor.
  constructor.
  constructor.
  apply IHstep.
...
(** Having seen the subject reduction property, it is reasonable to
   ...
   no, you cannot assume the opposite property holds.  Counter example:
   tif ttrue tzero -> tzero.  Subce ttrue is a bool and tzero is a nat, 
   tif ttrue tzero is not typed.


Tzero has a definitive type, but
   the expression on the left hand of the arrow does not.  Therefore,
   the opposite implication of type cannot be proven.
...
does NOT remain true. Doesn't make sense to take successor of a bool.
...
(** Suppose, instead, that we add this new rule to the [step] relation: 
  ...
  Deterministic- NOT stays the same.  Since tif ttrue t2 t3 can now step to 
  either t2 or t3.

  Progress- We didn't change the typing relation, so it stays true.

  Preservation- stays same- since we're not changing the typing relation and
  only changing the stepping relation, when it steps, it will still be preserved.
[]
*)
