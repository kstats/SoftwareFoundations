...
Proof. 
  ...
  Case "->".
    intros H2.
    apply WHILE_true_nonterm in H2.
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileLoop : forall (st st' st'' : state) (b1 : bexp) (c1 : com),
                  ...
                  (WHILE b1 DO c1 END) / st || st''
  | E_Havoc: forall (st:state)(X:id)(n:nat), (HAVOC X)/ st || update st X n
...
Proof. 
  ...
  Case "->". 
    ...
    assert (update (update st X n) Y n0 = update (update st Y n0) X n). 
       apply functional_extensionality.
       intros x. SearchAbout update. apply update_permute. (* remember (update st Y n0) as y'.
       apply update_neq. apply update_neq with (st := (update st Y n0)). apply update_permute. 
       SearchAbout update. rewrite -> update_neq in H8. *)(**
       unfold beq_nat.
   **) Admitted.
