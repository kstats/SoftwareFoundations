...
Theorem rev_exercise1 : forall (l l' : list nat),
     ...
     l' = rev l.
Proof.
  intros l l' H. rewrite->H. symmetry.  
...
Proof.
  intros n h. inversion h. destruct n.
  Case "n = 0".
    reflexivity.
  Case "n = S n".
    inversion h.
...
Proof.
  ...
  Case "n = S n'".
    ...
    SCase "m = 0".
      inversion h0.
    SCase "m = S m".
      repeat rewrite <-plus_n_Sm in h0.
      simpl in h0.
      repeat apply last_resort in h0.
      apply IHn' in h0.
      rewrite->h0.
      reflexivity.
Qed.
      (*Code that didn't work: apply f_equal in eq.
      inversion eq.
      rewrite->H0.
      rewrite->IHn'.
      reflexivity.
   (*repeat rewrite -us_n_Sm in eq.*)
    (*symmetry in H0.
    rewrite <- plus_n_Sm in H0.*)
    simpl h2. symmetry in h2. apply IHn' in h2.
    inversion H0.  apply IHn' in H1.  rewrite -> H1. reflexivity.


      inversion h2.
      repeat rewrite<-plus_n_Sm in h2.
      inversion h2.
       apply IHn' in H1. . apply IHn' in H1. inversion H0. *) 
...
(* advanced exercise, not required. *)
(** [] *)
...
Proof.
  ...
  Case "l = O".
    ...
    reflexivity.
  Case "l = S l'".
    intros n H. simpl. rewrite<-H. apply IHl. reflexivity.
Qed.
...
Theorem combine_split : forall X Y (l : list (X * Y)) l1 l2,
  ...
  combine l1 l2 = l.
Proof.
  (* optional exercise, not required *) Admitted.
(** [] *)
...
intros f b.
 ...
 Case "f b = true".
   ...
   SCase "f true = false".
    ...
    (*subcase when f false = true...coq won't allow two subcases*)
    apply H0.
    destruct b.
    symmetry.
    apply H1.
(*subcase when f false = false*)
    symmetry. apply H2.
  SCase "f true = false".
    destruct (f false) eqn:H2.
    destruct b.
...
Qed.
    (*
  intros f b. destruct(f b) eqn:H0.
  Case "f b = true".  
    destruct (f true) eqn:H1.
    SCase "f true = true".
      apply H1.
...
Lemma override_lemma_1: forall n m,
    n = m -> S n = S m.
Proof.
    intros n m H. rewrite -> H. reflexivity.
Qed.

Lemma override_lemma_2: forall n m,
    beq_nat n m = true -> n = m.
Proof.
    ...
    Case "n = S n'". 
      intros m H. destruct m as [|m'].
      SCase "m = O".
        inversion H. apply override_lemma_1. apply IHn'.
        simpl in H. apply H.
Qed.
