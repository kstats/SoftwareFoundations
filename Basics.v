...
Theorem andb_eq_orb : 
 ...
 Proof.  intros b c.
   destruct b. 
   intros H.
   rewrite<-andb_lemma with c. (* since c is the changing variable*)
   rewrite <- H.
   destruct c. reflexivity. reflexivity. simpl. 
   intros H. rewrite->H.
   reflexivity. Qed.
...
Inductive bin : Type :=
  | B : bin
  | Tb : bin -> bin (*inductively defines Tb twice binary and Tbo twice binary
  plus one*)
  | Tbo : bin -> bin .
...
Fixpoint increment (n : bin): bin :=
  match n with
  |B => Tbo B (*0 * 2 + 1 = 1 in a decimal sense*)
  |Tb n' => Tbo n'  (* 2b + 1 is form of Tbo*)
  |Tbo n' => Tb (increment(n')) (* 2b +1 +1 = 2(b+1)*)
  end. 

Fixpoint convert_to_unary (n : bin): nat :=
  match n with
  |B => 0
  |Tb n' => 2 * convert_to_unary(n')
  |Tbo n' => 2 * convert_to_unary(n') + 1
  end.

(*the following are test cases used to test the binary data type*)
Theorem Test_one : (convert_to_unary (Tbo B) = 1).
Proof. reflexivity. Qed.

Theorem Test_two : (convert_to_unary ((Tb (Tbo B))) = 2).
Proof. reflexivity. Qed.

Theorem Test_three : (convert_to_unary ((Tbo (Tbo B))) = 3).
Proof. reflexivity. Qed.

Theorem Test_four : (convert_to_unary ((Tb (Tb (Tbo B)))) = 4).
Proof. reflexivity. Qed.

Theorem Test_five : (convert_to_unary (Tbo (Tb (Tbo B))) = 5).
Proof. reflexivity. Qed.

Theorem Test_six : (convert_to_unary (Tb (Tbo (Tbo B))) = 6).
Proof. reflexivity. Qed.

Theorem Test_seven : (convert_to_unary (Tbo (Tbo (Tbo B))) = 7).
Proof. reflexivity. Qed.

Theorem Test_eight : (convert_to_unary (Tb (Tb (Tb (Tbo B)))) = 8).
Proof. reflexivity. Qed.

Theorem Test_nine : (convert_to_unary (Tbo (Tb (Tb (Tbo B)))) = 9).
Proof. reflexivity. Qed.

Theorem Test_ten : (convert_to_unary (Tb (Tbo (Tb (Tbo B)))) = 10).
Proof. reflexivity. Qed.
...
(** don't have to do this last exercise *)
