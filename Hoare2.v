...
(**have to use mult if you wanted to formalize this in coq**)
...
I wasn't sure if my logic came across in the coding, so here's a quick explanation:
min (x-1)(y-1) = (min x y ) -1, so in the 6th from the top, I simplify min (X-1)(Y-1)
to (min X Y) -1 and cancel the one out with the Z + 1 on the other side.
*)
