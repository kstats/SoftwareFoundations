...
Proof.
  intros b.
  induction b; (* w/o semi colon only does induction once...*)
  try reflexivity;
  try (simpl; repeat rewrite-> optimize_0plus_sound; reflexivity).
  try reflexivity;
  Case "Beq".
    simpl. try reflexivity. rewrite->IHb. try reflexivity.
  Case "BAnd".
    simpl. try reflexivity. rewrite->IHb1. try reflexivity.
    rewrite->IHb2.
    try reflexivity.
Qed.
(** [] *)
...
Inductive bevalR: bexp -> bool -> Prop :=
 ...
 | E_BEq : forall e1 e2 e3 e4,
     aevalR e1 e3 -> aevalR e2 e4 -> BEq e1 e2 || beq_nat e3 e4
 | E_BLe : forall e1 e2 e3 e4,
     aevalR e1 e3 -> aevalR e2 e4 -> BLe e1 e2 || ble_nat e3 e4
 | E_BNot : forall e1 e2,
     e1 || e2 -> BNot e1 || negb e2
 | E_BAnd : forall e1 e2 e3 e4,
     e1 || e3 -> e2 || e4 -> BAnd e1 e2 || andb e3 e4
...
(*PROVE EQUIVALENCE HERE*)
Theorem equiv_bevalR : forall e1 e2,
  bevalR e1 e2 <-> beval e1 = e2.
Proof.
split.
  intro H.
  induction H; simpl.
  ...
  reflexivity.
    ...
    assert (aeval e1 = e3). 
    induction H ; simpl; try (rewrite IHaevalR1) ; try (rewrite IHaevalR2);
    try reflexivity.
  assert (aeval e2 = e4).
  induction H0 ; simpl; try (rewrite IHaevalR1) ; try (rewrite IHaevalR2);
    try reflexivity.
  rewrite H1.
  rewrite H2.
  reflexivity.
...
Proof.
  ...
  intros H2.
  unfold update.
  remember (eq_id_dec x1 x2) as h'.
  destruct x1. destruct x2.

  destruct h'.
  symmetry in Heqh'.
  rewrite->e in Heqh'.
  subst. subst. rewrite->e. reflexivity.
  reflexivity.
Qed.
...
Proof.
 ...
 intros H.
  ...
  unfold update.
  subst.
  remember (eq_id_dec x1 x3) as h1'.
  remember (eq_id_dec x2 x3) as h2'.

  destruct h1';
  destruct h2';
  try reflexivity; simpl.
  subst. contradict H.  reflexivity. Qed.
...
intros st st' contra. unfold loop in contra.
  remember (WHILE BTrue DO SKIP END) as h2.
  ceval_cases (induction contra) Case; try inversion Heqh2; subst; simpl.
  Case "E_WhileEnd". 
    inversion H.
  Case "E_WhileLoop". 
...
Inductive no_whilesR: com -> Prop :=
  | Ind_SKIP : no_whilesR SKIP
  | Ind_ASS : forall n1 n2, no_whilesR (n1 ::= n2)
  | Ind_SEQ : forall n1 n2, no_whilesR n1 -> no_whilesR n2 -> no_whilesR (n1 ;; n2)
  | Ind_IF : forall (n3 : bexp) (n1 n2 : com), no_whilesR n1 -> no_whilesR n2 ->
     no_whilesR(IFB n3 THEN n1 ELSE n2 FI)
  .
...
Proof.
  ...
  inversion H. subst.
    assert (exists st' : state, c1 / st || st').
      ...
      apply H1. 
      inversion H0.
    assert (exists st' : state, c2 / x || st').
      apply IHc2. 
      apply andb_true_iff in H1. 
      apply H1. 
      inversion H3.
  ...
  intros st.
    inversion H. subst.
    remember (beval st b) as h'.
    destruct h'.
    assert (exists st', c1 / st || st').
      apply IHc1. apply andb_true_iff in H1. apply H1.
      inversion H0.
      exists x. apply E_IfTrue. symmetry. assumption. assumption.
    assert (exists st', c2 / st || st').
      apply IHc2. apply andb_true_iff in H1. apply H1.
      inversion H0.
      exists x. apply E_IfFalse. symmetry. assumption. assumption.
  Case "while".
    intros st.
    inversion H.
Qed.
...
(* Executing prog1++prog2 is the same as executing prog1 then executing prog2. *)
...
Proof.
intros st stack prog1 prog2.
generalize dependent st.
generalize dependent stack.
generalize dependent prog2.
induction prog1.
  
intros st stack prog2.
  simpl. reflexivity.

  intros prog2 stack st. 
  simpl.

  
  Admitted. (* unfold s_execute. reflexivity. apply IHprog1'.

apply APlus. reflexivity.
      (simpl; apply IHprog1' with ((a :: prog1) := prog1); assumption).
  intros st. 
  induction prog1.
  Case "prog1 = []".
    reflexivity.
  Case "prog1 = h::t".
    intros.
    simpl.
     destruct (s_execute st stack prog1);
      apply IHprog1.
    rewrite->IHprog1. reflexivity.
    Admitted.
*)
 (*
    destruct a.
    destruct a;
     try apply IHprog1.  apply IHprog1.
  
  induction prog2.
  rewrite->IHprog1.
  reflexivity.*)

Lemma s_compile_correct_lemma : 
forall (st : state) (e : aexp) (stack : list nat), 
s_execute st stack (s_compile e) = (aeval st e) :: stack.
Proof.
intros st e. aexp_cases (induction e) Case.
Case "ANum".
reflexivity.
reflexivity.
intros.
...
rewrite->IHe1. simpl. rewrite->s_execute_compose. simpl.
  (* apply IHe1. reflexivity.
remember (APlus e1 e2) as x.
rewrite->IHe1.
  intros.
  unfold ::.
  apply s_execute_compose.*)
