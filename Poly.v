...
The type baz has no elements.  It is only inductively 
pointing to more baz, meaning it has no real elements
in the type.
[] *)
...
this exercise was one star and optional, so not required.
*)
...
Definition prod_uncurry {X Y Z : Type}
  (f : X -> Y -> Z) (p : X * Y) : Z :=
  (* advanced exercise, not required *) admit.
...
Proof.
  (* advanced exercise, not required *) Admitted.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter(fun h => (andb (evenb h) (negb (ble_nat h 7)))) l.
(* fun fact, this definition doesn't work when 7 and h are switched
on ble_nat because of the order in which ble_nat processes....*)
...
Definition partition {X : Type} (test : X -> bool) (l : list X)
                     : list X * list X :=
  (filter (fun h => (test h)) l, filter (fun h2 => negb(test h2)) l).
...
(** Show that [map] and [rev] commute.  You may need to define an
    auxiliary lemma. *)
Lemma map_rev_lemma : forall (X Y: Type) (f: X -> Y) (l : list X) (x : X),
...
Proof.
  ...
  Case "l = nil".
    reflexivity.
  Case "l = h :: t'".
    simpl. rewrite->map_rev_lemma. rewrite-> IHl. reflexivity.
Qed. 
(** [] *)
...
Theorem test_fold_map : forall (X Y:Type) (f : X -> Y) (l : list X),
